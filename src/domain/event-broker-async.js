...
class EventBrokerImpl extends EventBroker {
  ...
  on (
    ...
    /** @type {eventHandler} */
    class EventCallbackWrapper extends AsyncResource {
      constructor (handler, eventData) {
        super(handler)
      ...
      handleEvent () {
        if (
          ...
          const ctx = this
          if (delay > 0) setTimeout(ctx.runInAsyncScope(ctx.handler), delay, eventData)
          else ctx.runInAsyncScope(ctx.handler, ctx, ctx.eventData)
        } else {
    ...
    function invokeAsyncWrapper () {
      return new EventCallbackWrapper(handler).handleEvent()
    }
    ...
    handlers.set(eventName, [invokeAsyncWrapper])
    this.subscriptionCb(eventName)
    return sub
  }
