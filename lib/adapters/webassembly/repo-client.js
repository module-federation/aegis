...
const RepoClient = {
  ...
  octoGet(entry) {
    ...
    return new Promise(function (resolve, reject) {
      octokit.request('GET /repos/{owner}/{repo}/contents/{filedir}?ref={branch}', {
        owner,
        repo,
        filedir,
        branch
        ...
        resolve({
          ...
          asBase64Buffer: () => buf,
          toUint16Array: () => new Uint16Array(buf.buffer, buf.byteOffset, buf.length / Uint16Array.BYTES_PER_ELEMENT)
        });
   ...
   */
  async fetch(entry) {
    if (/^https:\/\/api.github.com.*/i.test(entry.url)) return this.octoGet(entry);
