...
exports.WasmInterop = function (wasmExports) {
  ...
  function lift(fn, kv) {
    return liftArray(pointer => liftArray(pointer => liftString(new Uint32Array(memory.buffer)[pointer >>> 2]), 2, new Uint32Array(memory.buffer)[pointer >>> 2]), 2, callExportedFn(fn, kv) >>> 0).map(([k, v]) => ({
      [k]: v
    })).reduce((a, b) => ({
      ...a,
      ...b
  ...
  function lower(entries) {
    return lowerArray((pointer, value) => {
      store_ref(pointer, lowerArray((pointer, value) => {
  ...
  return Object.freeze({
    importWasmCommands() {
      ...
      return Object.keys(commandNames).map(command => {
        return {
          [command]: {
            command: model => callWasmFunction(command, {
              ...model,
              modelId: model.getId(),
    ...
    importWasmPorts() {
      ...
      return Object.keys(ports).map(port => {
        const [service, type, consumesEvent, producesEvent, callback, undo, forward] = ports[port].split(',');
        return {
