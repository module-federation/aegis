...
var _queryParamsMongo = _interopRequireDefault(require("query-params-mongo"));
...
const processQuery = (0, _queryParamsMongo.default)({
  autoDetect: [{
    valuePattern: /^null$/i,
    dataType: 'nullstring'
  }],
 ...
 */
class DataSourceMongoDb extends _datasource.default {
  ...
  async connection() {
    try {
      while (connections.length < (dsOptions.numConns || 1)) {
        const client = new _mongodb.MongoClient(this.url, this.mongoOpts);
        await client.connect();
        connections.push(client);
        client.on('connectionClosed', () => connections.splice(connections.indexOf(client), 1));
      }
  ...
  streamList({
    ...
    try {
      let pipeArgs = [];
      const serializer = new _stream.Transform({
        ...
        transform(chunk, _encoding, next) {
          // comma-separate
          if (this.first) this.first = false;else this.push(',');
      ...
      const paginate = new _stream.Transform({
        writableObjectMode: true,
        // start of array
        construct(callback) {
          this.push('{ ');
  ...
  processOptions({
    ...
    return {
      ...processQuery(query),
      ...options
    }; // options must overwite the query not otherwise
  }
  ...
  async list(param = {}) {
    ...
    try {
      ...
      if (writable && !query.__json) this.streamList({
        writable,
        serialize,
        transform,
        options
...
exports.DataSourceMongoDb = DataSourceMongoDb;