...
exports.ServiceMeshClient = void 0;
var _os = _interopRequireDefault(require("os"));
var _ws = _interopRequireDefault(require("ws"));
var _events = _interopRequireDefault(require("events"));
var _circuitBreaker = _interopRequireDefault(require("../../domain/circuit-breaker.js"));
var _serviceLocator = require("./service-locator.js");
var _fs = require("fs");
...
const activeHost = process.env.DOMAIN || configRoot.services.cert.domain || configRoot.general.fqdn;
...
const constructUrl = () => protocol && host && port ? `${protocol}://${host}:${port}` : null;
let uplinkCallback;
class ServiceMeshClient extends _events.default {
  constructor(url = null) {
    ...
    this.headers = {
      'x-webswitch-host': _os.default.hostname(),
      'x-webswitch-role': 'node',
      'x-webswitch-pid': process.pid
    };
  }
  services() {
    return this.options.listServices ? this.serviceList = this.options.listServices() : this.serviceList;
  }
  telemetry() {
    return {
      eventName: 'telemetry',
  ...
  async resolveUrl() {
    const locator = new _serviceLocator.ServiceLocator({
      name: this.name,
  ...
  async connect(options = {}) {
    ...
    this.ws = new _ws.default(this.url, {
  ...
  send(msg) {
    if (this.ws && this.ws.readyState === this.ws.OPEN && this.ws.bufferedAmount < 1) {
      const breaker = (0, _circuitBreaker.default)('webswitch', msg => {
        debug && console.debug({
          fn: this.send.name,
          msg
        });
  ...
  async saveSendQueue(sender) {
    try {
      ...
      const file = _path.default.join(process.cwd(), 'public', 'senderQueue.json');
      const size = await new Promise(resolve => (0, _fs.stat)(file, (err, stats) => resolve(stats.size)));
      if (size > 99999999) {
        console.log('queue backing store too large');
        return;
      }
      const storedQueue = JSON.parse(Buffer.from((0, _fs.readFileSync)(file, 'binary')).toString());
      const concatQueue = storedQueue.concat(this.sendQueue);
      const unsentQueue = await sender(concatQueue);
      (0, _fs.writeFileSync)(file, Buffer.from(JSON.stringify(unsentQueue)), 'binary');
    } catch (error) {
      console.error({
        fn: this.saveSendQueue.name,
        error
      });
    }
  }
