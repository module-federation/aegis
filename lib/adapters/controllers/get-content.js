...
function prettifyJson(json) {
  ...
  return json.replace(/("(\\u[a-zA-Z0-9]{4}|\\[^u]|[^\\"])*"(\s*:)?|\b(true|false|null)\b|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?)/g, function (match) {
    ...
    if (/^"/.test(match)) {
      ...
      cls = "<span style='color: lightgreen                                                                         '>";
...
function findLocalRelatedModels(modelName) {
  const localModels = _domain.default.getModelSpecs().map(s => s.modelName.toUpperCase());
  const spec = _domain.default.getModelSpec(modelName);
  const result = !spec?.relations ? [] : Object.keys(spec.relations).map(k => spec.relations[k].modelName.toUpperCase()).filter(modelName => localModels.includes(modelName));
  console.log({
    result
  });
  return result;
...
function getContent(httpRequest, content, defaultTitle) {
  const contents = content instanceof Array ? content : [content];
  try {
    if (!httpRequest.query.html) return {
      contentType: 'application/json',
      content
    };
    if (httpRequest.query.html) {
      const title = getResourceName(httpRequest, defaultTitle) || 'AEGIS FEDERATED MICROSERVICES';
      let html = `
          <!DOCTYPE html>
          <html>
          <head>
      ...
      contents.forEach(function (content) {
        ...
        Object.keys(content).forEach(key => {
          let val = content[key];
          if (typeof val === 'object') val = `<pre><code>${prettifyJson(JSON.stringify(val, null, 2))}</code></pre>`;
       ...
       */
      if (/config/i.test(httpRequest.path) && !Object.keys(httpRequest.query).includes('modelName') && !Object.values(httpRequest.query).includes('threads')) {
        const queryParams = Object.keys(httpRequest.query).map(k => `${k}=${httpRequest.query[k]}`);
        let queryText = '';
        queryParams.forEach(p => queryText += p + '&');
        html += '<div class="mb3">';
        _domain.default.getModelSpecs().forEach(s => {
          ...
          try {
            ...
            if (httpRequest.query.details === 'data') {
              ...
              related.forEach(rel => html += `<a href="${httpRequest.path}?${queryText}modelName=${s.modelName}&poolName=${rel}">View ${s.modelName} thread info for ${rel}</a><br>`);
            }
          } catch (error) {
            console.error(error);
          }
