...
class CdcCapture extends Map {
  get(target) {
    const targetName = target.constructor?.name || target.prototype?.constructor.name || target.modelName || target.getName();
...
function findChanges(from, to) {
  ...
  return {
    toObject: () => captureArray.map(i => ({
      [i.prop]: fromProxy[i.prop]
    })).reduce((a, b) => ({
      ...a,
      ...b
    }), {}),
...
function filterByChangeType({
  ...
  if (changeType) return keys.filter(key => changeTypes[changeType](from[key], to[key], changeTypeValue));
  return keys;
}
module.exports = {
  changeDataCapture,
