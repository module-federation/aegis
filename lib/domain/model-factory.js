...
const ModelFactory = {
   ...
   * @returns
   */
  getModelSpecs: () => [...modelFactories.values()].filter(spec => !spec.internal),
  /**
  ...
  getModelSpec: (model, options) => {
    if (!model) return;
    const name = typeof model === 'object' ? _model.default.getName(model) : model;
  ...
  getEvents: () => Object.keys(eventFactories).map(k => ({
    type: k,
   ...
   */
  updateModel: (model, changes) => _model.default.update(model, changes),
  /**
   *
   * @param {Model} model
   */
  deleteModel: model => _model.default.delete(model),
  /**
   ...
   * @param {Model} model
   */
  getModelId: model => _model.default.getId(model),
  /**
   * Get model's name
   */
  getModelName: model => _model.default.getName(model),
  /**
   * Get `eventName` value
   */
  getEventName: createEventName,
  EventTypes,
  getService: (modelName, datasource, broker) => _model.default.makeService({
