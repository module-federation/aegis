...
exports.getErrorLogs = exports.default = void 0;
...
function wasThresholdBreached(log, error, thresholds) {
  ...
  const entriesInScope = log.filter(entry => entry.time > Date.now() - threshold.intervalMs);
  ...
  const breach = callVolume > threshold.callVolume && errorRate > threshold.errorRate;
  console.debug({
...
function setStateOnError(log, error, options) {
  ...
  if (state === State.HalfOpen || state === State.Closed && wasThresholdBreached(log, error, options)) {
    return State.Open;
  }
  return state;
}
 ...
 * @param {string} error
 */
function logError(id, error, thresholds) {
...
const CircuitBreaker = function (id, protectedCall, thresholds) {
  ...
  const monitorErrors = function (eventName) {
    this.addListener && this.addListener(eventName, () => this.handleError(eventName), {
      singleton: true
    });
  };
  return {
    ...
    detectErrors(eventNames, emitter = null) {
      ...
      if (emitter instanceof _events.default) {
        errorList.forEach(error => emitter.on(error, () => logError(id, error, thresholds)));
      }
    }
  };
};
const getErrorLogs = () => logs;
