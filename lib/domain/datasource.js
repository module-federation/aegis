...
var _changeDataCapture = require("./util/change-data-capture");
/** change data capture */
...
class DataSource {
  ...
  handleChanges(id, data) {
    ...
    if (!prev) return data;
    const proxyClone = (0, _changeDataCapture.changeDataCapture)({
      ...prev
    }, this.changeHandlers());
  ...
  */
  ...
  handleCount(count) {
    ...
    if (key && value) {
      return {
        [key]: value,
        total: this.filterList({
          [key]: value
        }, this.generateList()).length
      };
    }
    return this.count();
  }
  ...
  filterList(query, list) {
    if (query) {
      ...
      if (typeof operands[operand] !== 'function') throw new Error('invalid query');
      const keys = Object.keys(query).filter(key => !['__cached', '__operand'].includes(key.toLowerCase()));
      if (keys.length > 0) {
        const filteredList = list.filter(obj => operands[operand](keys, key => obj[key] && new RegExp(query[key]).test(obj[key])));
        return filteredList;
      }
    }
    return list;
   ...
   */
  getCacheSizeBytes() {
    return this.countSync() * roughSizeOfObject(this.listSync({
      __limit: 1
    }));
  }
