...
var _ = require(".");
var _domainEvents = _interopRequireDefault(require("../domain/domain-events"));
var _asyncPipe = _interopRequireDefault(require("./util/async-pipe"));
...
function DistributedCache({
  ...
  async function streamCode(o) {
    ...
    if (!models.getModelSpec(modelName.toUpperCase())) {
      ...
      await (0, _.importRemoteCache)(modelName.toUpperCase());
      return o;
    }
    console.debug('...we do.');
    return o;
  ...
  async function handleDelete(eventName, modelName, event) {
    if (eventName === models.getEventName(models.EventTypes.DELETE, modelName)) {
   ...
   */
  const handleUpsert = (0, _asyncPipe.default)(streamCode, hydrate, save);
  ...
  async function createModels(event) {
    ...
    const service = (0, _useCases.UseCaseService)(modelName);
    const models = await Promise.all(event.args.map(async arg => {
  ...
  async function saveModels(event) {
    try {
      ...
      const datasource = datasources.getDataSource(event.relation.modelName.toUpperCase());
  ...
  const newModels = (0, _asyncPipe.default)(createModels, saveModels);
  ...
  function searchCache(route) {
    return async function (message) {
      try {
        ...
        const relatedModels = await _makeRelations.relationType[relation.type](model, datasources.getDataSource(relation.modelName), relation);
        console.debug({
          fn: searchCache.name,
          msg: 'related model(s)',
   ...
   * @param {*} internalName
   */
  const receiveSearchResponse = (responseName, internalName) => subscribe(responseName, updateCache(async event => broker.notify(internalName, event)));
   ...
   */
  const answerSearchRequest = (request, response) => subscribe(request, searchCache(event => publish({
    ...event,
    eventName: response,
   ...
   */
  const forwardSearchRequest = (internalEvent, externalEvent) => broker.on(internalEvent, event => publish({
    ...event,
   ...
   */
  const receiveCrudBroadcast = eventName => subscribe(externalCrudEvent(eventName), updateCache());

  /**
   *
   * @param {string} eventName
   */
  const broadcastCrudEvent = eventName => broker.on(eventName, async event => publish({
    ...event,
    eventName: externalCrudEvent(eventName)
  }));
  ...
  function listen() {
    ...
    const localModels = (0, _useCases.modelsInDomain)(_worker_threads.workerData.poolName);
    const relatedModels = [...modelSpecs.filter(spec => spec.relations).map(spec => Object.values(spec.relations)).flat(2).map(relation => relation.modelName).reduce((unique, modelName) => unique.add(modelName), new Set())];
    console.debug({
      relatedModels
    });
    const remoteRelated = relatedModels.filter(related => !localModels.includes(related));
    const remoteModels = [...new Set(
    // deduplicate
    modelSpecs.filter(m => m.relations) // only models with relations
    .map(m => Object.keys(m.relations).filter(
    // filter out existing local models
    k => !localModels.includes(m.relations[k].modelName.toUpperCase())).map(k => m.relations[k].modelName.toUpperCase())).reduce((a, b) => a.concat(b), []))];
    ...
    remoteModels.forEach(function (modelName) {
      ...
      forwardSearchRequest(internalCacheRequest(modelName), externalCacheRequest(modelName));
      // listen for external responses to forwarded requests
      receiveSearchResponse(externalCacheResponse(modelName), internalCacheResponse(modelName))
      //
      ;
      [
      // listen for CRUD events from related, external models
      models.getEventName(models.EventTypes.UPDATE, modelName), models.getEventName(models.EventTypes.CREATE, modelName), models.getEventName(models.EventTypes.DELETE, modelName)].forEach(receiveCrudBroadcast);
    });
    ...
    localModels.forEach(function (modelName) {
      ...
      answerSearchRequest(externalCacheRequest(modelName), externalCacheResponse(modelName))
      ...
      ;
      [models.getEventName(models.EventTypes.UPDATE, modelName), models.getEventName(models.EventTypes.CREATE, modelName), models.getEventName(models.EventTypes.DELETE, modelName)].forEach(broadcastCrudEvent);
    });
  }
