...
function connectEventChannel(eventPort) {
  try {
    ...
    eventPort.onmessage = msgEvent => broker.notify(msgEvent.data.eventName, msgEvent.data);

    // forward events to main
    broker.on('to_main', event => eventPort.postMessage(JSON.parse(JSON.stringify(event))));
  } catch (error) {
    console.error({
      fn: connectEventChannel.name,
      error
    });
  }
}
      ...
      try {
        if (msg.data?.modelName) {
          ...
          if (typeof domainPort !== 'function') {
            throw new Error(`no port ${msg.name} on model ${msg.data.modelName}`);
          }
          try {
