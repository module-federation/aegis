...
function getTimerArgs(args = null) {
  const timerArg = {
    calledByTimer: new Date().toISOString()
  };
...
function setPortTimeout(options) {
  ...
  const retry = async () => {
    ...
    await (0, _asyncError.default)(model[portName](...timerArgs.nextArg));
  };
...
function addPortListener(portName, portConf, broker, datasource) {
  if (portConf.consumesEvent) {
    ...
    async function listen(eventInfo) {
      ...
      console.log(`event ${eventInfo.eventName} fired: calling port ${portName}`, eventInfo);
      // invoke this port
      await (0, _asyncError.default)(model[portName](callback));
    }
 ...
 * @param {import('./event-broker').EventBroker} broker
 */
function makePorts(ports, adapters, broker, datasource) {
  if (!ports || !adapters) {
    return;
  }
  return Object.keys(ports).map(function (port) {
    ...
    const rememberPort = disabled ? false : addPortListener(portName, portConf, broker, datasource);
    ...
    async function portFn(...args) {
      ...
      try {
        ...
        const result = await adapters[port]({
          model: this,
          port,
          args
        });
        ...
        const model = rememberPort ? (await updatePortFlow.call(this, result, port)) || this : this;

        // Signal the next port to run.
        if (rememberPort) {
          console.log({
            producerEvent: portConf.producesEvent
          });
    ...
    return {
      ...
      async [port](...args) {
         ...
         */
        const breaker = (0, _circuitBreaker.default)(port, portFn, thresholds);

        // Listen for errors
        breaker.detectErrors([portRetryFailed(this.getName(), port), portTimeout(this.getName(), port)]);
