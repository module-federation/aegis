...
exports.default = makeRelations;
exports.relationType = void 0;
exports.requireRemoteObject = requireRemoteObject;
...
const relationType = {
  ...
  oneToMany: async (model, ds, rel) => {
    const filter = {
      [rel.foreignKey]: model.getId()
    };
    // retrieve from memory
    const memory = ds.listSync(filter);
    // retrieve from from external storage
    const externalMedia = await ds.list({
      query: filter
    });
    // return all
    if (memory.length > 0) return externalMedia.filter(ext => !memory.find(mem => mem.equals(ext))).concat(memory);
   ...
   * @returns
   */
  containsMany: (model, ds, rel) => Promise.all(model[rel.arrayKey].map(arrayItem => ds.find(arrayItem[rel.foreignKey]))),
  /**
   *
   * @param {import('.').Model} model
...
const updateForeignKeys = {
  ...
  async [relationType.oneToMany.name](fromModel, toModels, relation, ds) {
    return Promise.all(toModels.map(async m => {
  ...
  async [relationType.containsMany.name](fromModel, toModels, relation, ds) {
    toModels.map(model => model.update({
      [relation.foreignKey]: fromModel.getId()
    }));
  },
...
function isRelatedModelLocal(relation) {
  return require('.').default.getModelSpecs().filter(spec => !spec.isCached).map(spec => spec.modelName).includes(relation.modelName);
}
 ...
 */
function makeRelations(relations, datasource, broker) {
  if (Object.getOwnPropertyNames(relations).length < 1) return;
  return Object.keys(relations).map(function (relation) {
    const rel = {
      ...relations[relation],
      ...
      return {
        ...
        async [relation](...args) {
          ...
          if (!local) {
            ...
            await (0, _importRemotes.importRemoteCache)(rel.modelName);
          ...
          if (!models || models.length < 1) {
            ...
            const event = await requireRemoteObject(this, rel, broker, ...args);
            if (createNew) updateForeignKeys[rel.type](this, event.model, rel, ds);
