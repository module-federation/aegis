...
exports.reviveFunction = exports.replaceMap = exports.replaceFunction = exports.default = void 0;
const replaceMap = {
  on: 'serialize',
  key: '*',
...
const reviveFunction = {
  on: 'deserialize',
  key: 'function',
  type: 'string',
  value: (key, value) => eval(`(${value})`)
};
exports.reviveFunction = reviveFunction;
const type = {
  serialize: 'serialize',
...
function checkTypes(s) {
  return ['serialize', 'deserialize'].includes(s.on) && (['function', 'string'].includes(typeof s.key) || s.key instanceof RexExp) && (['string', 'number', 'function', 'object'].includes(s.type) || s.type instanceof RegExp || typeof s.type === 'function') && typeof s.value === 'function';
}
...
function validateSerializer(serializers) {
  const newSerializers = Array.isArray(serializers) ? serializers : [serializers];
...
function applies(serializer, key, value) {
  return typeApplies[typeof serializer.type](serializer, key, value) && keyApplies[typeof serializer.key](serializer, key, value);
}
