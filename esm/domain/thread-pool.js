...
exports.default = exports.ThreadPool = void 0;
 ...
 */
class Job extends _async_hooks.AsyncResource {
  constructor({
    jobName,
    jobData,
    modelName,
    ...
    super('Job');
    const store = new Map([..._.requestContext.getStore()]);
  ...
  stopTimer() {
    ...
    _.requestContext.getStore().set('threadDuration', this.duration);
 ...
 * jobs to complete.
 */
class ThreadPool extends _stream.EventEmitter {
  constructor({
    file,
    name,
    workerData = {},
  ...
  connectEventChannel(worker, channel) {
    ...
    port1.onmessage = async event => event.data.eventName && broker.notify('from_worker', event.data);
  }
  ...
  newThread({
    ...
    return new Promise((resolve, reject) => {
      _stream.EventEmitter.captureRejections = true;
      const eventChannel = new MessageChannel();
      const worker = new _worker_threads.Worker(file, {
        workerData
      });
      ...
      const thread = {
        ...
        run(job) {
          ...
          const unsubscribe = (eventName, callback) => this[channel].removeListener(eventName, callback);
          const messageFn = _async_hooks.AsyncResource.bind(result => {
            ...
            if (result.hasError) job.reject(result);else job.resolve(result);
            // reallocate thread
            pool.reallocate(this);
            job.dispose();
          });
          const errorFn = _async_hooks.AsyncResource.bind(error => {
            pool.jobTime(job.stopTimer());
            console.error({
              fn: this.run.name,
              error
          ...
          const exitFn = _async_hooks.AsyncResource.bind(exitCode => {
            pool.jobTime(job.stopTimer());
            console.warn('thread exited', {
              thread: this,
              exitCode
            });
            unsubscribe('message', messageFn);
   ...
   */
  async startThreads() {
    for (let i = 0; i < this.minPoolSize(); i++) this.freeThreads.push(await this.startThread());
    return this;
  }
  ...
  async stopThreads(reason) {
    for (const thread of this.threads) console.warn(await this.stopThread(thread, reason));
  ...
  poolCanGrow(pool = this) {
    ...
    return pool.capacityAvailable() && Object.values(conditions).some(satisfied => satisfied());
  }
  ...
  async fireEvent(event) {
    const eventScopes = {
      host: event => broker.notify('to_worker', {
        ...event,
      ...
      response: event => this.runJob(event.eventName, event, this.name, {
  ...
  reload(pool = this) {
    return new Promise((resolve, reject) => {
      pool.close().notify(poolClose).drain().then(pool => pool.stopThreads('reload')).then(pool => pool.startThreads()).then(pool => pool.open().bumpDeployCount().notify(poolOpen)).catch(reject).then(resolve);
    });
  }
  destroy(pool = this) {
    return new Promise((resolve, reject) => {
      pool.close().notify(poolClose).drain().then(pool => pool.stopThreads('destroy')).catch(reject).then(resolve);
    });
  }
  ...
  async drain() {
    ...
    return new Promise((resolve, reject) => {
      if (this.noJobsRunning()) resolve(this);else {
        const timerId = setTimeout(() => reject(new Error('drain timeout')), 4000);
        this.once(NOJOBS, () => {
...
const ThreadPoolFactory = (() => {
  ...
  function getBroadcastChannel(poolName) {
    ...
    const channel = new _worker_threads.BroadcastChannel(poolName);
  ...
  function calculateMaxThreads(options) {
    ...
    return Math.floor(_os.default.cpus().length / (0, _.totalServices)() || DEFAULT_THREADPOOL_MAX);
  }
  ...
  function createThreadPool(poolName, options) {
    ...
    const file = options.file || options.eval || _path.default.resolve(__dirname, 'worker.js');
    try {
  ...
  async function removeUndeployedPools() {
    const allModels = _.default.getModelSpecs().map(spec => spec.modelName);
    await Promise.all(listPools().filter(poolName => !allModels.includes(poolName.toUpperCase())).map(poolName => destroy(threadPools.get(poolName))));
  }
  ...
  function listen(cb, poolName, eventName) {
    if (poolName === '*') threadPools.forEach(pool => pool.on(eventName, cb));else {
      const pool = [...threadPools.values()].find(pool => pool.name.toUpperCase() === poolName.toUpperCase());
      if (pool) pool.on(eventName, cb);
    }
  }
  let monitorIntervalId;
  const poolMaxAbortTime = () => [...threadPools].reduce((max, pool) => max > pool[1].jobAbortTtl ? max : pool[1].jobAbortTtl, DEFAULT_TIME_TO_LIVE);
      ...
      threadPools.forEach(pool => {
        ...
        if (workWaiting > 0 && workersAvail < 1) {
          ...
          setTimeout(async () => {
            ...
            if (pool.jobQueueDepth() > 0 && pool.availThreadCount() < 1 && pool.totalTransactions() - pool.jobQueueDepth() === workCompleted) {
              ...
              for (const thread of pool.threads) {
                ...
                thread.run(new Job({
                  ...
                  resolve: result => result === timerId && clearTimeout(result) && (timerId = null) && pool.freeThreads.push(thread)
                }));
              }
            }
          }, pool.jobAbortTtl);
...
var _default = ThreadPoolFactory;
