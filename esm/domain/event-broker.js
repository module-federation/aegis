...
exports.default = exports.EventBroker = void 0;
var _event = _interopRequireDefault(require("./event"));
var _hash = _interopRequireDefault(require("./util/hash"));
...
function runHandler(eventName, eventData, handle) {
  ...
  if (debugHandler) console.debug('event fired', {
    eventName,
    eventData
...
function notify(eventName, eventData = {}, options = {}) {
  ...
  const data = typeof eventData === 'object' ? eventData : _event.default.create({
    args: eventData,
    eventName
  });
  let match = false;
  ...
  try {
    ...
    [...handlers].filter(([k]) => k instanceof RegExp && k.test(eventName)).forEach(([, v]) => v.f(fn => run(eventName, formattedEvent, fn)));
  } catch (error) {
...
class EventBrokerImpl extends EventBroker {
  ...
  on(eventName, handler, {
    ...
    const subscription = _event.default.create({
      eventName
    });
    ...
    const eventCallbackWrapper = async eventData => {
      const conditions = {
        ...
        priviledged: {
          applies: true,
          // have to check the data to know
          satisfied: event => !event || !event._options || !event._options.priviledged || event._options.priviledged === (0, _hash.default)(priviledged)
        },
        ignore: {
      ...
      if (Object.keys(conditions).every(key => {
        return !conditions[key].applies || conditions[key].satisfied(eventData);
      })) {
        if (once) this.off(eventName, eventCallbackWrapper);
        if (delay > 0) setTimeout(handler, delay, eventData);else handler(eventData);
      } else {
  ...
  serialize() {
    return JSON.stringify([...handlers].map(([k, v]) => ({
      [k]: v.map(fn => fn.toString())
    })), null);
  }
  getEvents() {
    return [...handlers];
...
var _default = EventBrokerFactory;
