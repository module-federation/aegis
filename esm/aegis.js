...
class RouteMap extends Map {
  find(path) {
    ...
    if (routeInfo) return {
      ...routeInfo,
      params: routeInfo.matcher(path).params
    };
  }
  set(path, method) {
    if (super.has(path)) {
...
function buildPath(ctrl, path) {
  return ctrl.path && ctrl.path[path.name] ? ctrl.path[path.name] : path(ctrl.endpoint);
}
...
const router = {
  autoRoutes(path, method, controllers, adapter, ports = false) {
    controllers().filter(ctrl => !ctrl.internal).forEach(ctrl => {
      if (ports) {
        if (ctrl.ports) {
...
async function handle(path, method, req, res) {
  ...
  const requestInfo = Object.assign(req, {
    params: routeInfo.params
  });
  try {
    // track this request
    requestContext.enterWith(new Map([['id', req.headers['idempotency-key'] || nanoid()], ['begin', Date.now()], ['user', req.user], ['res', res], ['path', path], ['headers', req.headers]]));
