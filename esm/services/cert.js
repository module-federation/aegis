...
function makeChallengeCreateFn(dnsProvider) {
  return async function challengeCreateFn(authz, challenge, keyAuthorization) {
    ...
    if (challenge.type === 'http-01') {
      ...
      console.log(`Create TXT record "${dnsRecord}" with value "${keyAuthorization}"`);
...
function makeChallengeRemoveFn(dnsProvider) {
  return async function challengeRemoveFn(authz, challenge, keyAuthorization) {
    ...
    if (challenge.type === 'http-01') {
      ...
      // dns-01
      const dnsRecord = `_acme-challenge.${authz.identifier.value}`;
      console.log(`Removing TXT record "${dnsRecord}" with value "${keyAuthorization}"`);
      const provider = await dnsProvider();
...
const directoryUrl = !/prod/i.test(process.env.NODE_ENV) ? acme.directory.letsencrypt.staging : acme.directory.letsencrypt.production;
