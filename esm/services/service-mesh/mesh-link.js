...
const DEBUG = /true/i.test(userConfig.services.serviceMesh.MeshLink.config.debug) || false;
const defaultCfg = {
  redis: {
    host: '127.0.0.1',
    port: 6379
...
const SharedObjEvent = {
  CREATE: async eventData => {
    ...
    return mlink.sharedObject.get(mid).then(so => {
  ...
  UPDATE: eventData => mlink.sharedObject.get(sharedObjects.get(eventData.modelName).mid).then(so => so.set('members', eventData.model.getId(), {
    ...JSON.parse(JSON.stringify(eventData.model))
  })),
...
function initSharedObject(broker) {
  if (!registerSharedObjEvents) {
    registerSharedObjEvents = broker => broker.on(/^externalCrudEvent_.*/, async (eventName, eventData) => SharedObjEvent[eventName.split('_')[1].substr(0, 6)](eventData), true);
