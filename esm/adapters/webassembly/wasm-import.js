...
exports.importWebAssembly = function (remoteEntry) {
  async function instantiate(module, imports = {}) {
    ...
    const adaptedExports = Object.setPrototypeOf({
      ...
      modelFactory(kv) {
        ...
        kv = __lowerArray((pointer, value) => {
          __store_ref(pointer, __lowerArray((pointer, value) => {
            __store_ref(pointer, __lowerString(value) || __notnull());
          }, 4, 2, value) || __notnull());
        }, 5, 2, kv) || __notnull();
        return __liftArray(pointer => __liftArray(pointer => __liftString(new Uint32Array(memory.buffer)[pointer >>> 2]), 2, new Uint32Array(memory.buffer)[pointer >>> 2]), 2, exports.modelFactory(kv) >>> 0).map(([k, v]) => ({
          [k]: v
        })).reduce((a, b) => ({
          ...a,
          ...b
      ...
      getPorts() {
        ...
        return __liftArray(pointer => __liftArray(pointer => __liftString(new Uint32Array(memory.buffer)[pointer >>> 2]), 2, new Uint32Array(memory.buffer)[pointer >>> 2]), 2, exports.getPorts() >>> 0).map(([k, v]) => ({
          [k]: v
        })).reduce((a, b) => ({
          ...a,
          ...b
      ...
      emitEvent(kv) {
        ...
        kv = __lowerArray((pointer, value) => {
          __store_ref(pointer, __lowerArray((pointer, value) => {
            __store_ref(pointer, __lowerString(value.toString()) || __notnull());
          }, 4, 2, value) || __notnull());
        }, 5, 2, kv) || __notnull();
        return __liftArray(pointer => __liftArray(pointer => __liftString(new Uint32Array(memory.buffer)[pointer >>> 2]), 2, new Uint32Array(memory.buffer)[pointer >>> 2]), 2, exports.emitEvent(kv) >>> 0);
      },
      getCommands() {
        // assembly/index/getCommands() => ~lib/array/Array<~lib/array/Array<~lib/string/String>>
        return __liftArray(pointer => __liftArray(pointer => __liftString(new Uint32Array(memory.buffer)[pointer >>> 2]), 2, new Uint32Array(memory.buffer)[pointer >>> 2]), 2, exports.getCommands() >>> 0).map(([k, v]) => ({
          [k]: v
        })).reduce((a, b) => ({
          ...a,
          ...b
      ...
      onUpdate(kv) {
        ...
        return __liftArray(pointer => __liftArray(pointer => __liftString(new Uint32Array(memory.buffer)[pointer >>> 2]), 2, new Uint32Array(memory.buffer)[pointer >>> 2]), 2, exports.onUpdate(kv) >>> 0);
      },
      onDelete(kv) {
  ...
  return initWasm().then(wasmExports => _.wasmAdapters[remoteEntry.type](wasmExports));
};